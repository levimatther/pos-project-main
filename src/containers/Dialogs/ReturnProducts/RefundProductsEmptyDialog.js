...
function ReturnProductsEmptyDialog(props) {
    ...
    function checkIfRefund() {
        return JSON.stringify(initialProducts) === JSON.stringify(products);
    }
    ...
    return (
        <Dialog onClose={props.handleCancel} open={props.open} classes={{paper: classes.paperDialog}} transitionDuration={_transitionDuration} TransitionComponent={Transition}>
                <div
                ...
                <div className='pl-20 pr-20 pt-20'>
                    <Button className={classes.refundBtn} fullWidth color='primary' variant='contained'
                            disabled={checkIfRefund()} onClick={() => setComplete(true)}>Refund</Button>
                </div>
                <CSSTransition
                    in={addNote}
...
export default ReturnProductsEmptyDialog;
